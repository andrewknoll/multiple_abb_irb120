...
Grid::Grid(ros::NodeHandle* nh, float x_origin, float y_origin, float width, float height, int hres, int vres) : nh(nh), x(x_origin), y(y_origin), w(width), h(height), hr(hres), vr(vres){
    grid = std::vector <std::vector <GazeboSphere> >(vres);
    for(int i = 0; i < vres; i++) {
        grid[i] = std::vector <GazeboSphere>(hres);
        for(int j = 0; j < hres; j++) {
            grid[i][j] = GazeboSphere(nh);
            grid[i][j].setPosition(calculateInitialPos(i, x_origin, width, hres), calculateInitialPos(j, y_origin, height, vres), 1);
        }
    }
}

const GazeboSphere& Grid::getNeighbour(int i, int j, int neighbour) {
    return grid[i][j];
}
